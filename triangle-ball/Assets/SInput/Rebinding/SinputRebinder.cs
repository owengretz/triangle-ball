	...
	public class SinputRebinder : MonoBehaviour {
		...
		void AddDevicePanel(InputDeviceType deviceType, string deviceName, int deviceSlotIndex){
            ...
            for (int c=0; c<controls.Length; c++){
                ...
                int player = int.Parse(controls[c].name.Substring(7, 1));
                if (player != currentPlayer)
                {
                    CreateHeader(newDevicePanelObj, player);
                ...
                Debug.Log(controls[c].name);

                string name = controls[c].name.Substring(9);


                newControlPanel.controlNameText.text = name;
				//Debug.Log(c);
				int controlIndex = c;
        ...
        void CreateHeader(GameObject newDevicePanelObj, int playerNumber)
        {
            if (playerNumber != 1)
            {
                GameObject blankPanelHeader = (GameObject)GameObject.Instantiate(ControlPanelPrefab);
                blankPanelHeader.name = "Blank";
                blankPanelHeader.transform.SetParent(newDevicePanelObj.transform.Find("ControlsPanel").transform);
                blankPanelHeader.transform.localScale = Vector3.one;
                controlPanel blankPanel = new controlPanel();
                blankPanel.controlPanelObj = blankPanelHeader;

                blankPanel.controlNameText = blankPanelHeader.transform.Find("ControlName").Find("ControlNameText").GetComponent<Text>();
                blankPanel.controlNameText.text = string.Empty;

                blankPanel.addInputButton = blankPanelHeader.transform.Find("AddInput").GetComponent<Button>();
                blankPanel.addInputButton.gameObject.SetActive(false);

                blankPanel.resetInputsButton = blankPanelHeader.transform.Find("ResetControl").GetComponent<Button>();
                blankPanel.resetInputsButton.gameObject.SetActive(false);
            }




            GameObject panelHeader = (GameObject)GameObject.Instantiate(ControlPanelPrefab);
            panelHeader.name = "Player " + playerNumber + " Controls";
            panelHeader.transform.SetParent(newDevicePanelObj.transform.Find("ControlsPanel").transform);
            panelHeader.transform.localScale = Vector3.one;
            controlPanel panel = new controlPanel();
            panel.controlPanelObj = panelHeader;

            panel.controlNameText = panelHeader.transform.Find("ControlName").Find("ControlNameText").GetComponent<Text>();
            panel.controlNameText.text = "Player " + playerNumber + " Controls";

            panel.addInputButton = panelHeader.transform.Find("AddInput").GetComponent<Button>();
            panel.addInputButton.gameObject.SetActive(false);

            panel.resetInputsButton = panelHeader.transform.Find("ResetControl").GetComponent<Button>();
            panel.resetInputsButton.gameObject.SetActive(false);
        }
